...
mod test {
    ...
    fn test_challenge() {
        ...
        assert_eq!(
            Vec::from(pn.as_bytes()).to_hex(),
            "f4aad10d3ba02cafb490c943c689c98895bd87197de841450dffb7ad28a17238"
        );
        ...
        let result = result.chain(fee.to_le_bytes());

        let lock_height: u64 = 60;

        assert_eq!(Vec::from(lock_height.to_le_bytes()).to_hex(), "3c00000000000000");

        let result = result.chain(lock_height.to_le_bytes()).result().to_vec().to_hex();
        // assert_eq!(result, "f5b611a5a70e8b692b1258864ebc14b7616151af56606c973ebf919ee35ff300");
        assert_eq!(
            result,
            "1afd4287bfc507e3bfbce398e7d6d9948e25bdbdc1caf61fdd19deb45c41272b"
        );
        // should be:
